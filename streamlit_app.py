...
logger.info(
    f"OPENROUTER_API_KEY present: {bool(openrouter_key)}; "
    f"prefix: {openrouter_key[:6] + '...' if openrouter_key else 'None'}"
)
...
st.session_state['role'] = current_user_role
...
elif filters.get('page') == "My Profile":
    render_profile_page(current_user_id)
    st.stop()
